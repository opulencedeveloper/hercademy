...
const PaymentSelection = () => {
  ...
  const commonButtonStyle =
    "h-full flex justify-center items-center rounded-[17px] font-semibold text-[16px] absolute top-0 bottom-0";
  return (
    <div className="flex flex-col justify-between gap-7 h-full">
      <div className="px-3 bg-secondary4 py-4 rounded-[8px]">
        <div className="flex relative w-[95%] mx-auto h-[41px] bg-white rounded-[17px] overflow-hidden">
          <button
        ...
        <div className="space-y-2.5 mt-5">
          {selectedPlan.map((planInfo, index) => (
            <div key={index} className="flex items-start space-x-2">
              ...
              <p className="text-[14px] font-medium">{planInfo}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div>
        <button className="bg-primary rounded-[8px] font-semibold text-[20px] text-white w-full py-2">
          {`${isFreeTrial ? "Start free Trial" : "Upgrade"}`}
        </button>
        {isFreeTrial && (
          <p className="text-[14] font-medium text-center mt-1">Upgrade anytime</p>
        )}
      </div>
    </div>
  );
